...
from app.v1.model import user_model, account_model, tarifa_model
...
def postgresql_connection():
    con = psycopg2.connect(f"user='{settings.db_user}' password='{settings.db_pass}' host='{settings.db_host}' port='{settings.db_port}'")
...
def pytest_sessionstart(session):
    ...
    with db:
        db.create_tables([user_model.Users, account_model.Accounts, tarifa_model.Tarifas])
